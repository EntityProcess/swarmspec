...
Must have passing verification evidence from swarmspec-verify before shipping. If no verification has been done in this session, stop and tell the user to run swarmspec-verify first.
...
All must pass with output as evidence. Do not skip this even if swarmspec-verify ran recently — code may have changed since.
